...
static inline uint64_t red_plantard_p1_ppr(uint32_t a)
{
	uint32_t c;
	c = a * 226050911;
	return (((c >> 16) + 1) * 19) >> 16;
...
/* Sample ZZ_p (for ppr), p=19 */
static const uint64_t sampler_zzp_check = 247;
void uni_sampler_zzp_1d_ppr(uint64_t *out, const size_t len, aes256ctr_ctx *ctx)
{
	size_t i;
...
static const uint64_t qbar = 441352763481784321ULL;
static const __uint128_t red_plantard_qbar_r = (((__uint128_t)6915422345779719679ULL)<<64)|18149506773181530113ULL;
...
/* Sample ZZ_qbar, qbar=441352763481784321 
 * The output is x*(-2**(-128)) mod qbar */
static const uint64_t sampler_zzqbar_check = 18095463302753157161ULL;
